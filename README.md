...
現在配布しているビルドはad-hoc署名済みですが、Appleのnotarization（公証）はまだ受けていません。そのため、初回起動時にmacOSによってブロックされることがあります。Marinaを一度開こうとした後、「システム設定」>「プライバシーとセキュリティ」を開き、このリポジトリとReleaseを信頼できる場合に限って「このまま開く」を選択してください。
